...
<%@page import="com.ETC.pojo.Purchase"%>
...
<html>
	...
	<script type="text/javascript">
		$(function() {
			...
			$.ajax({
				type:"post",
				url:"http://123.56.68.20:8080/Batik/purchase/getPrintData",
				data : data,
				dataType : "json",
				success : function(ret){
					...
					if (ret.status == "1") {
						...
						html = '';
						html += '<tr>'
							 +  '<td class="printName" colspan="13">'+ret.data.purchasePlanName +'</td>'
							 +  '</tr>'
							 +  '<tr>'
							 +  '<td class="printDis" colspan="13">'
							 +  '<span>制单时间：' + ret.data.purchasePlanName +'</span>'
							 +  '<span class="right">采购计划编码：'+ ret.data.purchasePlanCode +'</span>'
							 +  '<span>入库类型：' + ret.data.supplyWay +'</span>'
							 +  '</td>'
							 +  '</tr>'
							 +  '<tr>'
							 +  '<th width="2%" class="textCenter">序号</th>'
							 ...
							 +  '<tr>'
							 +  '<td class="textCenter">1</td>'
							 +  '<td>'+ ret.data.classificationCode +'</td>'
							 +  '<td>'+ ret.data.classificationNama +'</td>'
							 +  '<td>'+ ret.data.materialCode +'</td>'
							 +  '<td>'+ ret.data.materialName +'</td>'
							 +  '<td>'+ ret.data.specification +'</td>'
							 +  '<td>'+ ret.data.type +'</td>'
							 +  '<td>'+ ret.data.unit +'</td>'
							 +  '<td>'+ ret.data.requestMonth +'</td>'
							 +  '<td>'+ ret.data.requestDate +'</td>'
							 +  '<td>'+ ret.data.supplyQuantity +'</td>'
							 +  '<td>'+ ret.data.purchaseDate +'</td>'
							 +  '<td>暂无</td>'
							 +  '</tr>'
							 +  '<tr>'
							 +  '<td class="printOperator" colspan="13">'
